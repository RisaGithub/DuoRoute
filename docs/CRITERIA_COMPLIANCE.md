...
Среда: Ruby **3.4.10 через rbenv**, arm64-darwin25, Apple M4, 16 GiB RAM. Все числа ниже получены повторным запуском при переработке документации.

| Проверка | Фактический результат |
|---|---|
| `bin/ci` | Успешно: setup, RuboCop, проверка Markdown-ссылок, gem/importmap audit, Brakeman, Rails tests, seed replant |
| `bin/rails test` | **115 тестов, 32 973 assertions, 0 failures/errors/skips**; отдельный повтор после CI дал тот же результат |
| `RUBOCOP_CACHE_ROOT=/private/tmp/duoroute-rubocop-cache bundle exec rubocop` | 84 файла, нарушений нет |
| `bundle exec brakeman -q --no-pager` | 0 security warnings, 0 errors |
| `bundle exec bundler-audit check --update` | База обновлена: 1 242 записи, commit e7179ad21701894b75796c5ddd72e5fbfc446165; уязвимостей не найдено |
| `bin/rails zeitwerk:check` | Успешная загрузка классов |
| `RAILS_ENV=production SECRET_KEY_BASE_DUMMY=1 bin/rails assets:precompile` | Успешно; затем `bin/rails assets:clobber`, готовый манифест удалён |
| `bin/router demo --seed 42` | 10 операций; Approval 100%, Fallback 10%, Latency 61.6 с; это точный публичный сценарий |
| `ruby script/validate_10.rb routing_decisions.json` | 29 проверок, 0 ошибок, 0 предупреждений |
| Оригинальный валидатор организаторов | 29 проверок, 0 ошибок, 0 предупреждений; запущен напрямую из соседнего каталога |
| `bin/router evaluate-default` | 258 записанных результатов; reliability_snapshot; весь JSON совпал с предыдущим сохранённым экспериментом |
| `bin/router final --dry-run` | Ожидаемый exit 2, `file_not_found`; настоящей очереди нет, конкурсные файлы отсутствуют |
| `ruby script/check_markdown_links.rb` | 9 Markdown-файлов, 236 локальных ссылок и якорей, 0 ошибок; скрипт включён в CI, отдельно проверено обнаружение отсутствующего файла и якоря и пропуск примера внутри блока кода |
| HTTP smoke | 9 основных URL, 5 документов, страница запуска и операции — HTTP 200; исходные тексты всех пяти документов совпали |
| HTTP assets | Все 16 ресурсов загружаются; собственные CSS/JS побайтово совпадают с исходниками |
| Удалённые маршруты и закрытый список документов | Старые страницы, старые имена документов, неизвестный документ и попытка обхода пути возвращают 404; в Rails routes старых страниц нет |
| `git diff --check` | Без ошибок пробелов |

Assertions — отдельные утверждения тестов о результате. Проверки охватывают обязательные границы, 162 сочетания переходов, 3 seed × 7 стратегий и balanced, воспроизводимость обработанного префикса, точные деньги, будущую историю, повторную доставку задания и откат файлов при искусственной ошибке второго rename. Временные данные существующих тестов не являются финальной очередью.

Команда оригинального публичного валидатора в рабочем окружении:

```bash
ruby ../hackgenesis2026_task/google_drive_data/scripts/validate_10.rb routing_decisions.json
```

При первом CI sandbox не дал получить сведения о последней версии Brakeman. Повтор с сетевым доступом прошёл. Сам отдельный анализ Brakeman также завершился без предупреждений. Importmap audit пропускает локальный Chart.js без версии в pin; это ограничение проверки, а не подтверждение проверки этой библиотеки. Propshaft удаляет ссылку на отсутствующий source map у Chart.js, поэтому его выдача отличается от vendor-файла этой обработкой.

### Производительность и память

Команды запускаются последовательно, с полным объяснением решений, seed 42 и 100 000 строк истории:
...
| Очередь | Внешние провайдеры | Runner, с | Операций/с | Пик RSS, байт | Проверки числовых инвариантов |
|---|---:|---:|---:|---:|---:|
| 10 000 | 20 | 11.770 | 849.64 | 1 245 577 216 | 933 357 |
| 50 000 | 4 | 15.801 | 3164.40 | 1 727 512 576 | 1 452 664 |

RSS — занятая физическая память процесса; пик измерен `/usr/bin/time -l`, а не текущим RSS после расчёта. Во втором случае обработаны UTC-даты 30 и 31 июля. Отдельные 100 000 reserve/rollback и очистка RPM на 60-й секунде заняли 0.371/0.382 с соответственно.

Проверяются фактические вызовы: статус, реквизиты, маржа, min/max, daily, in-progress, RPM, отсутствие повторных попыток, сумма latency и неотрицательное состояние. Банковские ограничения дополнительно проверяются модульными и случайно сгенерированными тестами. Сериализация большого выходного файла не входит во время Runner; final может потребовать больше памяти для записи и повторной проверки. Это наблюдения, не гарантированная скорость или SLA.

Свидетельства хранятся отдельно от руководств: [10k](../artifacts/verification/BENCHMARK_BACKEND_10K.json), [50k](../artifacts/verification/BENCHMARK_BACKEND_50K.json), [HTTP smoke](../artifacts/verification/HTTP_SMOKE_BACKEND.json). Результат оценки default приведён выше. Старый отдельный benchmark и снимки экрана в `docs/screenshots/visual-qa` удалены как устаревшие; они доступны в истории Git и не подтверждают текущий интерфейс.

### Работающий сервер

Принадлежность процесса проверена по рабочему каталогу и команде Puma. Выполнен полный stop/start через `bin/dev -b 127.0.0.1 -p 3000`, готового манифеста assets нет.

HTTP 200: `/`, `/runs`, `/runs/new`, `/providers`, `/strategies`, `/generator`, `/about`, `/strategies/compare`, `/up`; дополнительно страница запуска и операции, все пять `/about/docs/...`. Основной smoke можно повторить:

```bash
for path in / /runs /runs/new /providers /strategies /generator /about; do
  curl --fail --silent --output /dev/null --write-out "$path %{http_code}\n" "http://127.0.0.1:3000$path"
done
```

Browser runtime вернул пустой список доступных браузеров. Визуальная проверка desktop/mobile **не выполнена**; HTTP, сравнение ресурсов и интеграционные тесты не заменяют её.
...
- **Память:** полное объяснение всей очереди хранится в памяти; запись и повторный разбор больших JSON требуют больше памяти, чем один Runner. Замеры не являются SLA.
...
- **Стратегия:** глобальный оптимум и точные доли на неизвестной очереди математически не гарантированы. Default выбран на ограниченных синтетических и публичных сценариях; выигрыш мал, часть показателей holdout хуже других кандидатов.
