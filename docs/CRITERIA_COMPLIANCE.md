...
Выполнено 133 сценариев и 1596 основных запусков: по 43 сценария development/validation/holdout и четыре отдельных публичных диагностики. В каждой основной выборке — 21 семейство × два seed и дополнительная очередь 10 000. Размеры: 10, 50, 100, 1 000, 10 000. Seeds: development 113/127, validation 223/227, holdout 331/347; для больших очередей используется первый seed + 1000. Публичные seeds 17/42/101/309 не участвуют в выборе. Генерация детерминирована; реальные providers/history участвуют в семействе real и публичной диагностике.

Проверены семь стратегий ТЗ, balanced и четыре ограниченных альтернативы: distribution (усилить заданные доли), equal_goals (одинаковые веса), count_latency (доли, conversion и latency без коррелирующих факторов нагрузки), latency_only (простой baseline времени). Дополнительно оценён auto_probe: выбор только из семи стратегий по development для каждого семейства, затем замороженный выбор на validation/holdout. Это диагностический селектор с известными метками семейства, не production-режим неизвестной очереди. Перебора тысяч весов нет.

Сценарии охватывают мелкие/крупные суммы, перекос банков, всплески нагрузки, daily/RPM/in-progress pressure, недоступность, нехватку реквизитов, банковские и денежные границы, высокий reject/expired, fallback, отсутствие и малую историю, частичные и нулевые volume targets, конфликт count/volume/turnover и недостижимые обязательства. Некорректные входы проверены отдельно тестами: отказ до расчёта для каждого кандидата, без фиктивных хороших метрик.

### Протокол и правило выбора

Входы глубоко копируются. Случайный ответ одной пары операция/провайдер зависит от seed, но не позиции в каскаде. Профили симуляции, цели и ограничения одинаковы для всех стратегий. Независимый SubmissionAudit проверяет каждый основной запуск по фактическим attempts и итоговому selected_provider, включая rejected/expired/fallback. Нарушений обязательных требований — 0. PairAudit и mutation-тесты обнаруживают подмену seed, входов, профиля, общих настроек, исходов, целей и атрибуции результата.

ТЗ не задаёт коэффициентов полезности между бизнес-метриками. Единого score нет. После допуска по hard constraints используется явно объявленный лексикографический порядок: worst-case regret неуспешности → средняя неуспешность → worst-case regret fallback → средний fallback → worst-case regret count → средняя ошибка count → применимый volume → применимый turnover → latency → комиссия. Сравниваются только одинаковые единицы одного показателя; отсутствующие цели исключены, явный ноль сохранён. Это инженерный приоритет надёжности для сравнения кандидатов.

Regret каждой метрики — разница с лучшим допустимым кандидатом в том же сценарии. Никакого сложения секунд, рублей и процентных пунктов нет. Средние — макро, каждый сценарий имеет одинаковый вес; дополнительно сохранены n, median, p90/p95, best/worst и population SD. Pareto оценивается по успешности, fallback, времени, заданным целям, комиссиям, давлению лимитов и нераспределённым операциям. Комиссия — оценка по одобренной сумме и provider_margin_pct, не известный договорный тариф.

Validation зафиксировала `amount_range` до первого holdout-расчёта; balanced занял второе место. Holdout проверяет зафиксированного кандидата относительно balanced, а не выбирает новые веса. Balanced — единственный проверенный статический кандидат с worst-case regret успешности не выше 0,1 п.п. и на validation, и на holdout. Кандидат `amount_range` имеет больший worst-case regret на holdout. Глобальная оптимальность не доказана.

### Сопоставление на holdout

| Кандидат | Approval среднее, % | Worst failure regret, п.п. | Fallback среднее, % | Latency среднее, с | Count error среднее, п.п. | Pareto из 43 |
|---|---:|---:|---:|---:|---:|---:|
| count_share | 98.662 | 0.2 | 30.13 | 41.836 | 83.842 | 36 |
| volume_share | 98.665 | 0.2 | 30.075 | 41.337 | 91.848 | 35 |
| cascade | 98.655 | 0.2 | 30.122 | 38.794 | 112.988 | 37 |
| amount_range | 98.665 | 0.2 | 30.03 | 39.883 | 111.52 | 36 |
| conversion | 98.658 | 0.2 | 30.118 | 45.466 | 126.79 | 19 |
| intensity | 98.663 | 0.2 | 30.063 | 41.18 | 84.164 | 39 |
| turnover_commitment | 98.657 | 0.2 | 30.087 | 39.067 | 114.42 | 38 |
| balanced | 98.667 | 0.1 | 29.98 | 42.403 | 87.722 | 34 |
| distribution | 98.667 | 0.2 | 29.994 | 42.644 | 87.495 | 30 |
| equal_goals | 98.669 | 0.1 | 30.08 | 41.567 | 100.848 | 35 |
| count_latency | 98.651 | 0.3 | 30.159 | 40.935 | 91.872 | 39 |
| latency_only | 98.655 | 0.2 | 30.122 | 39.622 | 114.755 | 37 |
| auto_probe | 98.665 | 0.2 | 30.13 | 41.25 | 86.697 | 36 |

### Среднее, медиана и худший результат `balanced`

| Выборка | Метрика | Среднее | Медиана | Худшее | p95 |
|---|---|---:|---:|---:|---:|
| validation | approval_rate_pct | 99.003 | 100 | 95.6 | 100 |
| validation | fallback_rate_pct | 31.254 | 12 | 100 | 98.7 |
| validation | average_latency_sec | 43.589 | 33.6 | 141.06 | 89.47 |
| validation | count_absolute_deviation_pp | 87.303 | 100 | 200 | 197.4 |
| validation | volume_absolute_deviation_pp | 57.632 | 55.88 | 134.74 | 100 |
| validation | turnover_gap_pct | 1293.642 | 87.567 | 4949.43 | 4949.43 |
| validation | commission_pct | 0.723 | 0.815 | 0.873 | 0.855 |
| holdout | approval_rate_pct | 98.667 | 100 | 90 | 100 |
| holdout | fallback_rate_pct | 29.98 | 12 | 100 | 99.2 |
| holdout | average_latency_sec | 42.403 | 34.44 | 146.52 | 79.03 |
| holdout | count_absolute_deviation_pp | 87.722 | 100 | 200 | 198.4 |
| holdout | volume_absolute_deviation_pp | 55.411 | 50 | 136.38 | 100 |
| holdout | turnover_gap_pct | 1275.214 | 74.558 | 4949.999 | 4949.999 |
| holdout | commission_pct | 0.722 | 0.812 | 0.88 | 0.86 |

### Парные величины выигрышей и потерь кандидата validation `amount_range` против balanced

| Выборка / метрика | Win/tie/loss | Средний выигрыш | Средний проигрыш | Максимальный проигрыш |
|---|---:|---:|---:|---:|
| validation / failure_rate_pct | 3/39/1 | 0.1 | 0.01 | 0.01 |
| validation / fallback_rate_pct | 2/34/7 | 0.7 | 0.693 | 2 |
| validation / average_latency_sec | 28/7/8 | 5.116 | 2.934 | 9.88 |
| validation / count_absolute_deviation_pp | 7/9/27 | 18.829 | 36.115 | 98 |
| holdout / failure_rate_pct | 0/42/1 | — | 0.1 | 0.1 |
| holdout / fallback_rate_pct | 4/36/3 | 0.25 | 1.05 | 2 |
| holdout / average_latency_sec | 23/8/12 | 5.707 | 1.908 | 7.64 |
| holdout / count_absolute_deviation_pp | 8/11/24 | 15.025 | 47.646 | 108 |

Каждая величина в таблице имеет единицы своей метрики. Нули количества поражений не означают одинакового распределения по другим целям. Полные хвосты, regret, результаты всех сценариев и Pareto-наборы сохранены в JSON, включая неудобные результаты.

### Почему balanced проигрывает публичному baseline

History, seed 42: balanced — approval 100%, fallback 20%, latency 133,9 с, count error 130 п.п.; count_share — 100%, 20%, 105,4 с, 40 п.п. Основная причина — предпочтение свободной ёмкости quickpay на фоне исходной дневной загрузки других провайдеров. Отключение load_safe или обнуление исходной нагрузки даёт 109,7 с / 50 п.п.; отключение conversion не меняет результат. Обратный tie-break также ничего не меняет. Разбор op_101/op_105/op_108, нормализация и влияние порядка — в [алгоритме](ALGORITHM.md#online) и public_diagnosis машинного отчёта. Специальных правил для этих операций нет.

### Границы вывода

Это проверка ограниченного набора, не доказательство математической оптимальности или прогноза финального трафика. Симуляция использует среднюю latency провайдера без отдельных распределений по статусам. Дизайн не перебирает все сочетания факторов; большие очереди при фиксированных дневных лимитах содержат много вынужденного fallback. Семейства и seeds связаны общей моделью генерации; статистическая значимость и перенос на неизвестное распределение не доказаны. Worst regret и ранжирование зависят от явно выбранного порядка целей. Auto не включён: известные метки семейства не дают готового селектора для неизвестного входа, устойчивого преимущества не установлено. Поведение при недоступном резерве — остановка всего расчёта, а не публикация частичного результата. Нет обещания точного достижения недостижимых долей.

Повторение: `ruby script/evaluate_default_final.rb tmp/repeated-study.json`; для сохранённого итогового исследования — `ruby script/replay_default_final.rb`. `bin/router evaluate-default` — отдельная диагностика, не источник конфигурации по умолчанию.
...
Benchmark `balanced`: 10 000 операций, 20 внешних провайдеров, 100 000 строк истории, seed 42, режим submission. Измеренный Runner — 8,367 с, peak RSS процесса — 732 987 392 байта. Это замер на конкретной среде, не SLA; предел 700 МБ для всего final pipeline не подтверждён. Данные сравнения — [исследование стратегий](../artifacts/verification/DEFAULT_STRATEGY_FINAL_EVALUATION.json).
...
Профили [full](../artifacts/verification/AUDIT_MEMORY_PROFILE.json) и [submission](../artifacts/verification/AUDIT_MEMORY_PROFILE_SUBMISSION.json) показывают удерживаемую память объяснения; она не равна peak RSS. Web сохраняет full. CLI final использует submission, сохраняя данные для независимого аудита.
