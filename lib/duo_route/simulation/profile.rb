  ...
  module Simulation
    class Profile
      ...
      def initialize(providers:, history:, config:, seed:, outcomes: nil)
        ...
        indexed = history.group_by { |row| row["payment_system"] }
        @profiles = providers.to_h { |provider| [ provider.fetch("payment_system"), profile(provider, indexed.fetch(provider["payment_system"], [])) ] }
      end
      ...
      def profile(provider, history)
        ...
        values = { "approved_rate" => approved, "rejected_rate" => [ 1 - approved - expired, 0 ].max, "expired_rate" => expired,
        ...
        %w[approved_rate rejected_rate expired_rate].each { |key| number!(key, values[key], 0, 1) }
        ...
        values.merge("source" => @source == "history" && !use_history ? "provider_snapshot" : @source,
          ...
          "available_history_records" => rows.length,
          "fallback_reason" => @source == "history" && !use_history ? "history_below_minimum_samples" : nil, "provider_snapshot_fallback" => @source == "history" && !use_history, "seed" => @seed)
      end
