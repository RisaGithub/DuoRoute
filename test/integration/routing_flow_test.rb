...
class RoutingFlowTest < ActionDispatch::IntegrationTest
  ...
  test "supporting pages render" do
    [ runs_path, providers_path, provider_path("vipay"), strategies_path, analytics_path, generator_path, methodology_path ].each do |path|
      get path
  ...
  test "navigation marks the current section on root and nested pages" do
    { root_path => root_path, new_run_path => runs_path, provider_path("vipay") => providers_path,
      strategies_path => strategies_path, analytics_path => analytics_path,
      generator_path => generator_path, methodology_path => methodology_path }.each do |path, active_path|
      get path
      assert_response :success, path
  ...
  test "dashboard names and timestamps the latest completed run" do
    ...
    assert_response :success
    assert_select ".page-head .eyebrow", text: "Данные последнего завершённого запуска"
    assert_select ".page-head h1", text: "Последний запуск · Latest snapshot"
